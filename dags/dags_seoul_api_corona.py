...
with DAG(
    ...
    tb_corona19_count_status = SeoulApiToCsvOperator(
        ...
        file_name='TbCorona19CountStatus_{{data_interval_end.in_timezone("Asia/Seoul") + macros.datetime.strftime(macros.timedelta(hours=8)) | ds_nodash }}.csv' # 해당명으로 파일 생성
    )
    ...
    tv_corona19_vaccine_stat_new = SeoulApiToCsvOperator(
        ...
        file_name='tvCorona19VaccinestatNew_{{data_interval_end.in_timezone("Asia/Seoul") + macros.datetime.strftime(macros.timedelta(hours=8)) | ds_nodash }}.csv'
    )
