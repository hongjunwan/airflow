...
with DAG(
    ...
    def insrt_postgres(postgres_conn_id, **kwargs):
        ...
        with closing(postgres_hook.get_conn()) as conn:
            with closing(conn.cursor()) as cursor:
                ...
                msg = 'hook insrt 수행'
                sql = 'insert into py_oprt_drct_insrt values (%s,%s,%s,%s);'
    ...
    insrt_postgres_with_hook = PythonOperator(
        ...
        op_kwargs={'postgres_conn_id':'conn-db-postgres-custom'} # airflow에 등록해놓은 connection 가져와서 insrt_postgres 함수에 값 전달
    )
